...
func init() {
	InitializeDB()
	err := db.AutoMigrate(&financial_servicev1.JournalLineORM{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&financial_servicev1.JournalLineORM{}) fail: %s", err)
	}
}
...
func Test_journalLineORMQuery(t *testing.T) {
	...
	err = _do.Create(&financial_servicev1.JournalLineORM{})
	if err != nil {
		t.Error("create item in table <journal_lines> fail:", err)
	}

	err = _do.Save(&financial_servicev1.JournalLineORM{})
	if err != nil {
		t.Error("create item in table <journal_lines> fail:", err)
	}

	err = _do.CreateInBatches([]*financial_servicev1.JournalLineORM{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <journal_lines> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*financial_servicev1.JournalLineORM{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&financial_servicev1.JournalLineORM{}, 0, 1)
	if err != nil {
...
func Test_journalLineORM_CreateRecord(t *testing.T) {
	...
	for i, tt := range JournalLineORMCreateRecordTestCase {
		t.Run("CreateRecord_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.CreateRecord(tt.Input.Args[0].(financial_servicev1.JournalLineORM))
...
func Test_journalLineORM_UpdateRecordByID(t *testing.T) {
	...
	for i, tt := range JournalLineORMUpdateRecordByIDTestCase {
		t.Run("UpdateRecordByID_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.UpdateRecordByID(tt.Input.Args[0].(int), tt.Input.Args[1].(financial_servicev1.JournalLineORM))
