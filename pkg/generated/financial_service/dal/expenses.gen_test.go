...
func init() {
	InitializeDB()
	err := db.AutoMigrate(&financial_servicev1.ExpenseORM{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&financial_servicev1.ExpenseORM{}) fail: %s", err)
	}
}
...
func Test_expenseORMQuery(t *testing.T) {
	...
	err = _do.Create(&financial_servicev1.ExpenseORM{})
	if err != nil {
		t.Error("create item in table <expenses> fail:", err)
	}

	err = _do.Save(&financial_servicev1.ExpenseORM{})
	if err != nil {
		t.Error("create item in table <expenses> fail:", err)
	}

	err = _do.CreateInBatches([]*financial_servicev1.ExpenseORM{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <expenses> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*financial_servicev1.ExpenseORM{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&financial_servicev1.ExpenseORM{}, 0, 1)
	if err != nil {
...
func Test_expenseORM_CreateRecord(t *testing.T) {
	...
	for i, tt := range ExpenseORMCreateRecordTestCase {
		t.Run("CreateRecord_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.CreateRecord(tt.Input.Args[0].(financial_servicev1.ExpenseORM))
...
func Test_expenseORM_UpdateRecordByID(t *testing.T) {
	...
	for i, tt := range ExpenseORMUpdateRecordByIDTestCase {
		t.Run("UpdateRecordByID_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.UpdateRecordByID(tt.Input.Args[0].(int), tt.Input.Args[1].(financial_servicev1.ExpenseORM))
