...
func init() {
	InitializeDB()
	err := db.AutoMigrate(&social_servicev2.TagsORM{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&social_servicev2.TagsORM{}) fail: %s", err)
	}
}
...
func Test_tagsORMQuery(t *testing.T) {
	...
	err = _do.Create(&social_servicev2.TagsORM{})
	if err != nil {
		t.Error("create item in table <tags> fail:", err)
	}

	err = _do.Save(&social_servicev2.TagsORM{})
	if err != nil {
		t.Error("create item in table <tags> fail:", err)
	}

	err = _do.CreateInBatches([]*social_servicev2.TagsORM{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <tags> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*social_servicev2.TagsORM{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&social_servicev2.TagsORM{}, 0, 1)
	if err != nil {
...
func Test_tagsORM_CreateRecord(t *testing.T) {
	...
	for i, tt := range TagsORMCreateRecordTestCase {
		t.Run("CreateRecord_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.CreateRecord(tt.Input.Args[0].(social_servicev2.TagsORM))
...
func Test_tagsORM_UpdateRecordByID(t *testing.T) {
	...
	for i, tt := range TagsORMUpdateRecordByIDTestCase {
		t.Run("UpdateRecordByID_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.UpdateRecordByID(tt.Input.Args[0].(int), tt.Input.Args[1].(social_servicev2.TagsORM))
