...
const ActionableInsightsViewContext = createContext<string>(
  'ActionableInsightsViewContextDefaultValue',
);

export type ActionableInsightsViewProps<T> = {
...
export type ActionableInsightsViewState = {
 ...
 */
export class ActionableInsightsView<T> extends Component<
  ActionableInsightsViewProps<T>,
  ActionableInsightsViewState
> {
  ...
  render(): ReactNode {
    ...
    return (
      <ActionableInsightsViewContext.Consumer>
        {(contextValue) => (
          <div role="presentation" className={className} ref={this.myRef}>
            {greeting} from ActionableInsightsView! Context Value:{' '}
            {contextValue}
            <p>Counter: {counter}</p>
