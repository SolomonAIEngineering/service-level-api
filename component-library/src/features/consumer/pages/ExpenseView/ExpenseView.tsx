...
const ExpenseViewContext = createContext<string>(
  'ExpenseViewContextDefaultValue',
);

export type ExpenseViewProps<T> = {
...
/**
 * @class ExpenseView
 * @extends {Component<ExpenseViewProps, ExpenseViewState>}
 *
 * @description
 ...
 */
export class ExpenseView<T> extends Component<
  ExpenseViewProps<T>,
  ExpenseViewState
> {
  ...
  constructor(props: ExpenseViewProps<T>) {
    super(props);
    this.state = {
      counter: 0,
    };
  ...
  render(): ReactNode {
    ...
    return (
      <ExpenseViewContext.Consumer>
        {(contextValue) => (
          <div role="presentation" className={className} ref={this.myRef}>
            {greeting} from ExpenseView! Context Value: {contextValue}
            <p>Counter: {counter}</p>
