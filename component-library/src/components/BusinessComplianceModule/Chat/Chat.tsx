...
import { ChatBody, Conversation, Message } from 'src/types/business-ai/chat';
        ...
        if (!plugin) {
          ...
          body = JSON.stringify({
            ...
            googleAPIKey: pluginKeys
              ...
              ?.requiredKeys.find(
                (key: { key: string }) => key.key === 'GOOGLE_API_KEY',
              )?.value,
            googleCSEId: pluginKeys
              .find((key) => key.pluginId === 'google-search')
              ?.requiredKeys.find(
                (key: { key: string }) => key.key === 'GOOGLE_CSE_ID',
              )?.value,
          });
        }
            ...
            {selectedConversation?.messages.length === 0 ? (
              ...
              <>
                ...
                {selectedConversation?.messages.map(
                  (message: any, index: Key | null | undefined) => (
                    <MemoizedChatMessage
                      messageIndex={0}
                      key={index}
                      message={message}
